      ...
      <div className="p-8 md:p-10">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-ink-dim">
            Industry credentials from AWS Academy and Cisco — the ones that took
            real study.
          </p>
        </div>

        <ul className="mt-9 grid grid-cols-2 gap-x-6 gap-y-9 md:grid-cols-4">
              ...
              >
                <div className="relative flex aspect-square w-full max-w-[9rem] items-center justify-center">
                  ...
                  >
                    <Image
                      src={badge.image}
                      alt={`${badge.name} badge — ${badge.issuer}`}
                      placeholder="blur"
                      sizes="9rem"
              ...
              <li key={badge.name}>
                {badge.file ? (
                  <a
                    ...
                    aria-label={`View credential: ${badge.name} (${badge.issuer}), opens PDF in a new tab`}
                  >
                    {body}
                    <span className="mt-3 flex items-center justify-center gap-1 text-xs font-medium text-tq-400 opacity-0 transition-opacity group-hover:opacity-100 group-focus-visible:opacity-100">
                      View credential
